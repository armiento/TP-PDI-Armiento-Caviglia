"""
Editor Interactivo de Imagenes de Frutas - INTERFAZ DROPDOWN
===========================================================
...
def _cb(_val):
    """Callback para los sliders estandar."""
    _necesita_actualizar[0] = True


def mouse_handler(event, x, y, flags, param):
    """Manejador del mouse para botones y Menu Desplegable flotante."""
    global _estado_navegacion
    if event == cv2.EVENT_LBUTTONDOWN:
        
        # 1. SI EL MENU ESTA ABIERTO: Detectar que opcion toco de la lista vertical
        ...
        # 2. BOTONES DEL ENCABEZADO (Cuando el menu esta cerrado)
        if 12 <= y <= 38:
            if 10 <= x <= 110:     # Boton << ANTERIOR
                _estado_navegacion["indice"] = (_estado_navegacion["indice"] - 1) % _estado_navegacion["total"]
                _necesita_actualizar[0] = True
            elif 120 <= x <= 220:  # Boton SIGUIENTE >>
                _estado_navegacion["indice"] = (_estado_navegacion["indice"] + 1) % _estado_navegacion["total"]
                _necesita_actualizar[0] = True
            elif 230 <= x <= 410:  # Boton central del MENU DESPLEGABLE
                _estado_navegacion["menu_abierto"] = True
                _necesita_actualizar[0] = True
            elif 420 <= x <= 510:  # Boton GUARDAR
...
def cargar_rutas_imagenes(carpeta_datos):
    ...
    print(f"[OK] {len(rutas)} imagenes listas.")
    return rutas


# ---------------------------------------------------------------------------
# Interfaz Grafica: Ventana de Parametros Especificos
# ---------------------------------------------------------------------------
def crear_controles(modo):
    """Recrea los sliders especificos sin duplicar el control de modo."""
    try:
